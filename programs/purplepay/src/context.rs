...
pub struct Initialize<'info> {
    ...
    /// [`wormhole::post_message`] only needs it to be read-only.8
...
#[derive(Accounts)]
#[instruction(chain: u16)] //Tells solana what chain emitter is in
pub struct RegisterEmitter<'info> {
    ...
    #[account(
        init_if_needed,
        payer = owner,
        seeds = [ForeignEmitter::SEED_PREFIX, &chain.to_le_bytes()[..]],
        bump,
...
#[derive(Accounts)]
#[instruction(vaa_hash: [u8; 32])] //Tells Solana what the vaa_hash of message is
...
#[derive(Accounts)]
pub struct ReadReceivedMessage<'info> {
    #[account(
        seeds = [SEED_PREFIX_SENT, received.to_account_info().key().as_ref()],
        bump = [wormhole_message.bump]
    )]
    pub received: Account<'info, Received>,
